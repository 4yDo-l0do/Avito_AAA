...
def make_a_report(name_file:str='Corp_Summary.csv'):
    '''функция создает сводный отчет в виде словаря'''
    with open(name_file, 'r', encoding = 'utf-8') as f:
        reader = csv.reader(f, delimiter=';')
        next(reader)
        report={}
...
def print_report(name_file:str='Corp_Summary.csv'):
    '''функция печатает отчет'''
    report = make_a_report(name_file)
    for key, value in report.items():
        print(f'{key} : численность - {value[0]}, вилка зарплат - {value[1]}, средняя зп - {value[2]}')


def save_report(name_file:str='Corp_Summary.csv'):
...
def step():
    ...
    '''в параметры функций вписать название рабочего файла(относительным или абсолютным видом), если оставить без изменения, то будет использован фаил по умолчанию Corp_Summary.csv'''
    if options[option] == 1:
